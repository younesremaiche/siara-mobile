...
ALTER TABLE app.notifications
  ADD COLUMN IF NOT EXISTS event_type VARCHAR(50) NOT NULL DEFAULT 'INCIDENT_REPORTED_IN_ZONE',
...
WITH ranked_notifications AS (
  SELECT
    id,
    ROW_NUMBER() OVER (
      PARTITION BY user_id, report_id, channel
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_notifications_user_report_channel
  ON app.notifications (user_id, report_id, channel);

CREATE OR REPLACE FUNCTION app.notification_priority_from_severity(severity_hint SMALLINT)
RETURNS INTEGER
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT CASE
    WHEN COALESCE(severity_hint, 0) >= 3 THEN 1
    WHEN severity_hint = 2 THEN 2
    ELSE 3
  END;
$$;

CREATE OR REPLACE FUNCTION app.notification_danger_score_from_severity(severity_hint SMALLINT)
RETURNS INTEGER
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT CASE
    WHEN COALESCE(severity_hint, 0) >= 3 THEN 82
    WHEN severity_hint = 2 THEN 68
    ELSE 41
  END;
$$;

CREATE OR REPLACE FUNCTION app.alert_time_range_matches(
  time_range_type VARCHAR,
  custom_time_start TIME,
  custom_time_end TIME,
...
AS $$
  SELECT CASE
    WHEN COALESCE(time_range_type, 'all') = 'all' THEN TRUE
    WHEN time_range_type = 'day' THEN (timezone('Africa/Algiers', matched_at))::time >= TIME '06:00'
      AND (timezone('Africa/Algiers', matched_at))::time < TIME '22:00'
    WHEN time_range_type = 'night' THEN NOT (
      (timezone('Africa/Algiers', matched_at))::time >= TIME '06:00'
      AND (timezone('Africa/Algiers', matched_at))::time < TIME '22:00'
    )
    WHEN time_range_type = 'custom' AND custom_time_start IS NOT NULL AND custom_time_end IS NOT NULL THEN
      CASE
        WHEN custom_time_start <= custom_time_end THEN
          (timezone('Africa/Algiers', matched_at))::time >= custom_time_start
          AND (timezone('Africa/Algiers', matched_at))::time <= custom_time_end
        ELSE
          (timezone('Africa/Algiers', matched_at))::time >= custom_time_start
          OR (timezone('Africa/Algiers', matched_at))::time <= custom_time_end
      END
    ELSE TRUE
  END;
$$;
  ...
  FOR inserted_notification IN
    ...
    matching_alerts AS (
      ...
      WHERE ar.user_id IS NOT NULL
        AND (report_context.reported_by IS NULL OR ar.user_id <> report_context.reported_by)
...
DROP TRIGGER IF EXISTS trg_notify_on_report ON app.accident_reports;

CREATE TRIGGER trg_notify_on_report
AFTER INSERT ON app.accident_reports
FOR EACH ROW
EXECUTE FUNCTION app.create_notifications_from_report();
